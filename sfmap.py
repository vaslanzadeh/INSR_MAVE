#this is taken from sfmap.py file implemented in Enrich2
...
#extract position, wt_aa and variant_aa from aa_substitutions column
def get_PosWTaaVARaa(df):
    def extract_PosWTaaVARaa(aa_substitutions):
        pos = int(aa_substitutions[1:-1])
        wt_aa = aa_substitutions[:1]
        variant_aa = aa_substitutions[-1:]
        return wt_aa, pos, variant_aa

    df['wt_aa'], df['position'], df['variant_aa'] = zip(*df['aa_substitutions'].apply(extract_PosWTaaVARaa))
...
def sfmap_axes(
    ...
    .. csv-table::
        ...
        Aroma , Phenylalanine , Phe , F
        Aroma , Tryptophan , Trp , W
        Aroma , Tyrosine , Tyr , Y
    ...
    *sfmap_name* sets heatmap name based on FACS sorting experiment
...
#crossing those variants that not detected by pacbio/not mutagenised
#=====================================
#    for x,aa in enumerate (df.index):
#        for y in range((len(df.columns))):
#            if (str(y+28)+aa) in missing_aa_changes:
...
#this would add points in the middle of those squares that are missing in mutagenesis data
...
#=====================================
    ...
    if show_positions:
        ...
        if tall:
            ...
            ax.set_ylabel("Amino Acid Sequence Function Map \n"+ sfmap_name)
        if tall:
            ax.set_yticklabels([x+28 for x in list(df.columns)])
        else:
            ax.set_xticklabels([x+28 for x in list(df.columns)], rotation=90)
    ...
    rect = plt.Rectangle(
        ...
        len(df.columns),
        0.2,
        color='white',  # You can set the color to whatever you prefer
        transform=ax.transData,
    )
    ax.add_patch(rect)
    ...
    *kwargs* is the dictionary of additional optional or required parameters to
    ...
    if tall:
        ...
        if title is not None and show_colorbar:
            ...
            #width_ratios=[len(df.columns), 0.02], height and width of colorbar
            #wspace=0.001 space between heatmap and colorbar
    ...
    if title_ax is not None:
        title_ax.text(s=title, x=0.5, y=1.0, ha="center", va="top", size="small")
        title_ax.axis("off")
